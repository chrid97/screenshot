    ...
    while (!WindowShouldClose()) {
            ...
            for (int y = top; y <= bottom; y++) {
                for (int x = left; x <= right; x++) {
                    ...
                    if (outer && !inner) {
                        ...
                        preview_buffer[index + 0] = stroke_color.r;
                        preview_buffer[index + 1] = stroke_color.g;
                        preview_buffer[index + 2] = stroke_color.b;
                        preview_buffer[index + 3] = stroke_color.a;
                    }
                }
            }
        ...
        if (IsMouseButtonReleased(MOUSE_LEFT_BUTTON)) {
            ...
            case ACTION_RECTANGLE: {
                ...
                for (int y = top; y <= bottom; y++) {
                    for (int x = left; x <= right; x++) {
                        ...
                        if (outer && !inner) {
                            int index = (y * image.width + x) * BYTES_PER_PIXEL;
                            image.pixels[index + 0] = stroke_color.r;
                            image.pixels[index + 1] = stroke_color.g;
                            image.pixels[index + 2] = stroke_color.b;
                            image.pixels[index + 3] = stroke_color.a;
                        }
                    }
                }
            ...
            case ACTION_CAPTURE: {
                ...
                // uint8_t *cropped_image_pixels = (uint8_t *)malloc(screenshot_byte_count(&image));
                //
                // for (int y = top; y < bottom; y++) {
                //     unsigned char *src = image.pixels + (y * image.width + left) *
                //     BYTES_PER_PIXEL; unsigned char *dst =
                //         cropped_image_pixels + ((y - top) * image.width * BYTES_PER_PIXEL);
                //     memcpy(dst, src, image.width * BYTES_PER_PIXEL);
                // }
                //
                // Image cropped = image;
        ...
        //         bool clicked =
        //             IsMouseButtonDown(MOUSE_BUTTON_LEFT) && CheckCollisionPointRec(mouse,
        //             button);
        //
